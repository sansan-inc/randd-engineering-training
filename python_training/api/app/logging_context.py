...
# ELB側でコネクションを切った後にアプリ側でも処理を終わらせる
...
class LoggingContextRoute(APIRoute):
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            ...
            before = time.time()
            try:
                response = await self._execute_request(request, original_route_handler, request_record)
            finally:
    ...
    async def _execute_request(self, request: Request, route_handler: Callable, record: dict) -> Response:
        try:
            ...
            if exc.status_code == status.HTTP_404_NOT_FOUND and exc.detail == "Not Found":
    ...
    async def _wait_for_request(self, request: Request, route_handler: Callable) -> Response:
        try:
            return await asyncio.wait_for(route_handler(request), timeout=REQUEST_TIMEOUT)
